...
```{r}
head(df_tbl9  <- as.data.frame(html_table(read_html(url_9))[[2]]),5)
head(df_tbl10 <- as.data.frame(html_table(read_html(url_10))[[2]]),5)
head(df_tbl34 <- as.data.frame(html_table(read_html(url_34))[[2]]),5)
```
...
test3$Month_Yr <- format(as.Date(test3$Month_Year), "%Y-%m")

glimpse(test3)

x <- as.yearmon("2021-03")
x

ggplot(test3, aes(x=Month_Year, y = Rate, color = Gender, group = Gender)) +
  geom_line() +
  geom_point(size = 4, shape = 21)  # Also use a point with a color fill
```

```{r}
p <- ggplot(test3, aes(x =Month_Year , y = Rate))+
...
```{r}
p4 <- ggplot() + 
  geom_bar(aes(y = Rate, x = Month_Year, 
...
#### Table 34
```{r}
table341<-anti_join(table34,table34[1:2,])
table341<-anti_join(table341,table341[2:8,])

```

```{r}
names(table341)[2] <- "Total"
names(table341)[3] <- "Job Sekkers"
names(table341)[4] <- "Contacted Employer Directly"
names(table341)[5] <- "Sent out resumes or filled out applications"
names(table341)[6] <- "Placed or answered ads"
names(table341)[7] <- "Contacted Friends or Relatives"
names(table341)[8] <- "Public Employment Agency"
names(table341)[9] <- "Private Employment Agency"
names(table341)[10] <- "Other"
names(table341)[11] <- "Average number of method used"
...
```{r}
#table341<-anti_join(table341,table341[1,])
```

```{r}
Men_Job_Search<-table341[1:4,]
Women_Job_Search<-table341[5:9,]
```
...
```{r}
NewTable34<-table341[1:10,1:2]
```

```{r}
Menjobsituation<-NewTable34[1:5,]

```

```{r}
library(data.table) 
dcast (melt(as.data.table(Menjobsituation), id.vars = c("Job losers and persons who completed temporary jobs(1)", "Job leavers","Reentrants","New entrants")))
