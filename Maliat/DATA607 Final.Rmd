...
The data from `Plot 1: Unemployment Rates by Occupation and Gender`seems to support the theory that women were more significantly impacted by unemployment in 2020 in virtually all industries than men. In 2021 women still struggled more than their counterpart, but not as notably.
...
`Plot 2: Unemployment Rates by Occupation and Gender` also seems to support this with exception of specific industries such as `Food preparation and serving related`.

```{r class.source = "fold-hide"}
ggplot(df_tb30_full, aes(x=factor(Occupation_Label), y = (Rate),color = Gender,
                         group = Gender, fill=Gender)) +
  geom_col(position="dodge") +
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 90, hjust=1))+
  labs(title = 'Plot 2: Unemployment Rates by Occupation and Gender'
       ,subtitle = "03/2020-03/2021",
       x = "Month", y = "Rate")
```

However when running `summary()` on our data, we see an low [`R-squared`](https://blog.minitab.com/en/adventures-in-statistics-2/regression-analysis-how-do-i-interpret-r-squared-and-assess-the-goodness-of-fit#:~:text=R%2Dsquared%20is%20a%20statistical,multiple%20determination%20for%20multiple%20regression.&text=100%25%20indicates%20that%20the%20model,response%20data%20around%20its%20mean.) value of `0.0118`. This indicates that our data does not fit well our regression model. 
...
This fact is supported when running `plot()`, which results in various charts that we cannot used to support whether a relationship between unemployment and gender.
...
We see that both male and females saw there largest unemployment spike in early March, women consistently having a higher rate monthly until about September 2020.
...
Additonally, when broken down into age groups, we can see `18 to 19 years`, `16 to 17 years` and `35 to 44 years` of age where the highest for women and far exceeded the rates for men in the early months.
...
Our results supports the title for the the article which inspired us, since it was published August 5,2020. At this point in the pandemic women were losing or leaving there jobs at a alarming rate. The concerns raised at the time were valid, and was at least 1 month away from producing data to supported the contrary. With the pandemic coming to a close (hopefully), we believe the trend should lean more favorably in women returning to the workforce.
