...
names(df_tb10marital)[1] <- "Marrital Label"
df_tb10marital$`Marrital Label`<- c("Married Men Spouse Present","Married Women Spouse Present")
...
df_tb10maritalu<-
  ...
  df_tb10maritalm%>%
    mutate(Rate2 = 100-Rate)%>%
      select(`Marrital Label`,Gender,Month_Year,Rate = Rate2) %>% 
        lapply(function(x){ 
        gsub("Married", "Unmarried", x)})%>%
          lapply(function(x){ 
        gsub("Spouse Present", "", x)}))

names(df_tb10maritalu)[1] <- "Marrital Label"
...
```{r class.source = "fold-hide"}
  ggplot(df_tb10marital, aes(x=factor(Month_Year), y = (Rate),color = `Marrital Label`,
     group = `Marrital Label`, fill=`Marrital Label`)) +
     geom_col(position="dodge") +
     theme_minimal()+
     theme(axis.text.x = element_text(angle=90, hjust=1))+
