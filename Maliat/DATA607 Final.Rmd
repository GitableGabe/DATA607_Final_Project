...
library(ggpubr)
df_tb30_full %>%
  arrange(Rate) %>%    # First sort by val. This sort the dataframe but NOT the factor levels
  mutate(Occupation_Label=factor(Occupation_Label, levels=Occupation_Label)) %>%   # This trick update the factor levels
  ggplot( aes(x=Occupation_Label, y=Rate)) +
    geom_segment( aes(xend=ccupation_Label, yend=0)) +
    geom_point( size=4, color=Gender) +
    coord_flip() +
    theme_bw() +
    xlab("")
...
```{r}
df_tb10marital<-df_tb10[42:45,]
df_tb10maritalc<-df_tb10marital[-c(1, 4), ]
names(df_tb10maritalc)[1] <- "Marrital Label"
...
```{r}
df_tb10maritalc[1, 1] = "Married Men Spouse Present"
df_tb10maritalc[2, 1] = "Married Women Spouse Present"
```
### Transposing dataset and adding new column
```{r}
Gender1<-c("Male","Female")
df_tb10maritalc$Gender<-Gender1
...
df_tb10maritalnew <- as.data.frame(df_tb10maritalc %>% 
    gather("Month_Year", "Rate", 2:14)%>%
      lapply(function(x){ 
        gsub(",", "", x)}))
...
```{r}
NewGraph<-ggplot(df_tb10maritalnew, aes(x=factor(Month_Year), y = (Rate),color = Gender,
                          group = Gender, fill=Gender)) +
