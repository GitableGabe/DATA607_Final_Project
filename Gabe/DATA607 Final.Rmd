...
- [ ] Your project has a recognizable “data science workflow,” such as the <span style="color:blue">OSEMN</span> workflow or Hadley Wickham’s Grammar of Data Science. [Example: First the data is acquired, then necessary transformations and clean-up are performed, then the analysis and presentation work is performed] 
...
```
<font size="11" color = "navy blue">Table 9 Data</font>

```{r class.source = "fold-hide"}
head(df_tb10 <- as.data.frame(html_table(read_html(url_10))[[2]]),5)%>%
  reactable(highlight = TRUE, pagination = FALSE, height = 450)
```
<font size="11" color = "navy blue">Table 10 Data</font>
...
This $tidy$ data is the baseline used for our evaluation of the comparison between unemployment rates of men and women, during the beginning month until present day, of the pandemic. 
...
Using [**ZevRoss**: Know Your Data | *Easy multi-panel plots in R using `facet_wrap()` and `facet()_grid()` from ggplot2](http://zevross.com/blog/2019/04/02/easy-multi-panel-plots-in-r-using-facet_wrap-and-facet_grid-from-ggplot2/) as a point of reference, we continue to transform the data.\newline
Specifically:
...
When sorted, it is easy to see that the highest unemployment rates overall, did occur among women between the ages of $16\ to\ 19$, during the months of $March\ and\ April\ of\ 2020$, by a small margin over men $18\ to\ 19$ in $March\ \&\ April\ 2020$
...
From here, the data is transformed in a month over month format in order to assess the data as a trend throughout the year. The functions used to create this data frame included, piping `%>%`, `slice()`, `gather()` and `mutate()`. Conversion of dates and counts with `as.yearmon()` & `as.numeric()` also followed.
The calculations for our month-over-month (`MoM`) values and rates were as follows:
...
df_tb9_bar <- df_tb9_tdy %>% 
  slice(c(14,26)) %>% 
    gather("Month_Year", "Count", 2:14)

df_tb9_bar$Month_Year<- as.yearmon(df_tb9_bar$Month_Year)

df_tb9_bar$Count <- as.numeric(gsub(",","",df_tb9_bar$Count))

df_tb9_bar <- df_tb9_bar %>% 
  group_by(Gender) %>% 
  mutate(MoM = (Count - lag(Count)) / lag(Count)) %>% 
  mutate(MoM_Rate =  round(MoM * 100, 1))
```

Or data sorted by rate is:

```{r, echo= FALSE,class.source = "fold-hide", results='hide'}
head(df_tb9_bar[with(df_tb9_bar, order(-MoM_Rate)),])%>%
  kbl(row.names = FALSE) %>%
  kable_styling(bootstrap_options = "striped",position = "center", full_width = T)
```
...
summary(lm(Count~Gender, df_tb9_bar))
```
...
(ggplot(df_tb9_bar, aes(x=factor(Month_Year), y = (MoM), color = Gender, group = Gender, fill=Gender)) +
...
```{r}
ggplot(df_tb9_bar, aes(x=factor(Month_Year), y=MoM, fill=Gender)) + 
  geom_col(position="dodge") +
...
p4 <- ggplot() + 
  geom_bar(aes(y = Rate, x = factor(Month_Year), 
    fill = Gender), data = df_tb10_trend,
