...
df_test2_2020 <- df_test2[,c(1,2,6,8)]
df_test2_2020$Total_Unemployed_Population_April_2020 <- as.numeric(gsub(",","",df_test2_2020$Total_Unemployed_Population_April_2020))
df_test2_2020$Men_Unemployment_Rate_2020 <- as.numeric(df_test2_2020$Men_Unemployment_Rate_2020)
df_test2_2020$Women_Unemployment_Rate_2020 <- as.numeric(df_test2_2020$Women_Unemployment_Rate_2020)
df_test2_2020 <- df_test2_2020 %>% 
  mutate(Male_Unemp_Pop_April2020= round(Total_Unemployed_Population_April_2020*(.01)*Men_Unemployment_Rate_2020)) %>% 
  mutate(Female_Unemp_Pop_April2020=round(Total_Unemployed_Population_April_2020*(.01)*Women_Unemployment_Rate_2020))
df_test2_2020$Year <- 2020


df_test2_2021 <- df_test2[,c(1,3,7,9)]
df_test2_2021$Total_Unemployed_Population_April_2021 <- as.numeric(gsub(",","",df_test2_2021$Total_Unemployed_Population_April_2021))
df_test2_2021$Men_Unemployment_Rate_2021 <- as.numeric(df_test2_2021$Men_Unemployment_Rate_2021)
df_test2_2021$Women_Unemployment_Rate_2021 <- as.numeric(df_test2_2021$Women_Unemployment_Rate_2021)
df_test2_2021 <- df_test2_2021 %>% 
  mutate(Male_Unemp_Pop_April2021= round(Total_Unemployed_Population_April_2021*(.01)*Men_Unemployment_Rate_2021)) %>% 
  mutate(Female_Unemp_Pop_April2021=round(Total_Unemployed_Population_April_2021*(.01)*Women_Unemployment_Rate_2021))
df_test2_2021$Year <- 2021

df_test2_2021 <- df_test2_2021[,c(1,2,5:7)]
df_test2_2020 <- df_test2_2020[,c(1,2,5:7)]
...
df_test2_rates <- df_test2[, c(1,6:9)] %>% 
  slice(-c(1,2,4,34:40)) %>% 
...
(ggplot(data = df_test, aes(Year, Rate, color = Gender, group = Gender, fill = Gender)) +
  ...
  labs(title = 'Unemployment Rates by Occupation and Gender'
       ,subtitle = "2020-2021",
       x = "Year", y = "Rate")+
  facet_wrap( ~ Occupation_Label, scales = "free_y", ncol = 4))
```
...
```{r}
a_gabes_test<-bind_rows(
              (cbind(Male = df_test2_2020$Male_Unemp_Pop_April2020,
                    Year = df_test2_2020$Year)),
              (cbind(Male = df_test2_2021$Male_Unemp_Pop_April2021,
                    Year = df_test2_2021$Year))
)
```
