...
```{r}
test2 <- df_tb10_tdy %>% 
  slice(c(16,17,19,22,23,24,25,28,29,31,34,35,36,37)) %>% #took out rows 14&26 (MF 16+)
  gather("Month_Year", "Rate", 2:14)

test2<-
as.data.frame(test2%>%
  lapply(function(x){gsub("Men, |Women, ", "", x)}))

test2$Month_Year<- as.yearmon(test2$Month_Year) 
test2$Rate <- as.numeric(test2$Rate)

#line graph by age group and gender
...
```{r}
#line graph
ggplot(test3, aes(x=factor(Month_Year), 
                  y = (Rate), color = Gender, group = Gender)) +
  geom_line() +
